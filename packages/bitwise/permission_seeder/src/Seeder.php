    ...
    protected function createPermissions()
    {
        $this->classes->each(function ($class) {
            ...
            if ($class->hasMethod('getPermissionDisplayName')) {
                ...
                $actionsAr = $instance->getPermissionActionsAr();
                $className = $class->getShortName();
                $group=(defined($class->getName().'::GROUP')) ? $class->getName()::GROUP : "master_data" ;
                $modelNameAr = (defined($class->getName().'::ARMODELNAME')) ? $class->getName()::ARMODELNAME : "" ;


                collect($instance->getPermissionActions())->each(function ($action) use ($modelNameAr,$actionsAr,$group,$class, $className, $modelName) {
                    $this->permissions[] =  [
                        'name' => $className . '-' . $action,
                        'model' => $className,
                        'group' => $group,
                        'display_name_en' => $action . ' ' . $modelName,
                        'display_name_ar' => $actionsAr["$action"] . ' ' . $modelNameAr,
                        // 'description_en' =>$action.' '.$modelName.'Desc EN',
                        // 'description_ar' => $action.' '.$modelName.'Desc AR',
                    ];
                });
            }
        });
